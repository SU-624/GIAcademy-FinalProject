...
public class EventScriptManager : MonoBehaviour
{
    ...
    [SerializeField] private ProfessorController m_SelectProfessor;     // 모든 강사를 돌면서 해금할 강사를 찾기 위한 리스트    [Space(5)]
    [Space(10)]
    ...
    string tempStr;                  // 스크립트 대사에 학원명을 플레이어가 지정한 이름으로 바꿔주기 위한 임시변수
    ...
    {
        string[] temptext;      // 전달받은 스크립트를 컷팅해서 담아주기 위한 배열
        ...
        if (tempScriptText != "")
        {
            if (tempScriptText.Contains("유저지정") == true)        // 유저지정 -> 플레이어가 지은 아카데미 이름으로 변경
            {
                string tempNameChangestr = tempScriptText.Replace("유저지정", PlayerInfo.Instance.AcademyName);

                temptext = tempNameChangestr.Split("/");        // 원본을 텍스트 갯수로 먼저 나눔
            }
            else
            {
                temptext = tempScriptText.Split("/");        // 원본을 텍스트 갯수로 먼저 나눔
            }
            ...
            for (int i = 0; i < temptext.Length; i++)
            {
                ...
                // 여기서 각 한줄의 대사에 포함 된 문장에 GI서당 라는 글이 들어가 있는지 체크한다
                //if(temp.Contains("GI서당") == true)
                //{
    ...
    public void PutOnScreenRewardPanel()
    {
        ...
        {
            // 리워드가 있을때 or 강사해금일때 or 회사 해금일때 보상 창 띄워주기
    ...
    public void IfClickEachStduentDepartmentIndexButton()
    {
        ...
        // 학과 인덱스를 눌렀을 때에는 선택된 학생 정보, 선택된 학생 목록중 표시도 끄지 않는다.
        ...
        switch (nowClickButton.name)
        {
            case "GameDesignerIndex":
            {
                ...
                {
                    if (nowTargetStudent != null)            // 현재 선택된 학생이 있을 때
                    {
        ...
        bool check)
    {
        ...
        else if (rewardID < 299)
        {
            foreach (var eventReward in AllOriginalJsonData.Instance.ChoiceEventReward)
            {
                ...
                {
                    rewardTaker[0] = (RewardTaker)Enum.Parse(typeof(RewardTaker), eventReward.RewardTaker1); // 보상1 수령자
                    rewardIndex[0] = eventReward.Reward1; // 보상1 인덱스
                    amount[0] = eventReward.Amount1; // 보상1 양
                    rewardTaker[1] = (RewardTaker)Enum.Parse(typeof(RewardTaker), eventReward.RewardTaker2); // 보상2 수령자
                    rewardIndex[1] = eventReward.Reward2; // 보상2 인덱스
                    amount[1] = eventReward.Amount2; // 보상2 양

                    // if (rewardIndex[0] == 600) // 교수 해금을 합니다
                    // {
    ...
    private string ApplyUnLockTeacherReward(bool check)
    {
        ...
        foreach (var eventReward in AllOriginalJsonData.Instance.OriginalSuddenEventDataList) // 전체 이벤트 탐색
        {
            if (eventReward.SuddenEventID == eventScheduleSystem.TodaySuddenEventList[0].SuddenEventID) // 지금 이벤트와 전체 이벤트 비교
            {
                ...
                {
                    if (m_SelectProfessor.professorData.ElementAt(j).Value.m_ProfessorID == teacherId) // 모든 강사 정보에서 해금할 강사를 찾는다.
                    {
        ...
        return tempTeacherInfo.m_ProfessorName;        // 강사 이름
    }
