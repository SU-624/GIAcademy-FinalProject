...
public class EventScheduleSystem : MonoBehaviour
{
    ...
    private bool SuddenEventDispatcher(SuddenEventTableData tempEvent, bool isAnyConditionMet)
    {
        ...
        if(tempEvent.SuddenEventID == 1105002)
        {
        ...
        for (int eventTypeindex = 3; eventTypeindex >= 1; eventTypeindex--)
        {
            ...
            switch (eventCondition)
            {
                ...
                case 200: // 1차 재화 N이상 보유
                    if (PlayerInfo.Instance.MyMoney >= amountN)
                        resultArr[eventTypeindex] = true;
                    break;

                case 201: // 2차 재화 N 이상 보유
                    if (PlayerInfo.Instance.SpecialPoint >= amountN)
                        resultArr[eventTypeindex] = true;
                    break;
                ...
                case 301: // 랭크 N등급 달성
                    // 랭크 정보 필요
                {
                    if ((int)PlayerInfo.Instance.CurrentRank - 1 >= amountN)
                    {
                        resultArr[eventTypeindex] = true;
                    }
                }
                ...
                case 303: // 유명도점수 N점 달성
                    // 유명도 점수 필요
                    if (PlayerInfo.Instance.Famous >= amountN)
                        resultArr[eventTypeindex] = true;
                    break;

                case 304: // 재화점수 N점 달성
                    // 재화점수 필요
                    if (PlayerInfo.Instance.Goods >= amountN)
                        resultArr[eventTypeindex] = true;
                    break;

                case 305: // 인재양성점수 N점 달성
                    //  인재양성 점수 필요
                    if (PlayerInfo.Instance.TalentDevelopment >= amountN)
                        resultArr[eventTypeindex] = true;
                    break;

                case 306: // 활동점수 N점 달성
                    // 활동 점수 필요
                    if (PlayerInfo.Instance.Activity >= amountN)
                        resultArr[eventTypeindex] = true;
                    break;

                case 307: // 운영점수 N점 이상
                    // 운영 점수 필요
                    if (PlayerInfo.Instance.Management >= amountN)
                        resultArr[eventTypeindex] = true;
                    break;
